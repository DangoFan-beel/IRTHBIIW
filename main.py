...
def get_shop_keyboard():
    ...
    for item in shop:
        kb.add(
            InlineKeyboardButton(f"{item['name']} - {item['price']}",
...
async def start(msg: types.Message):
    ...
    if user_id not in users:
        ...
        save_users()
    await msg.answer("Привет! Используй кнопки ниже ⬇️",
...
async def callback_handler(callback: types.CallbackQuery):
    ...
    if callback.data == "balance":
        ...
        if not shop:
            ...
            await callback.message.answer("🛒 Доступные товары:",
        ...
        if item:
            if users[user_id]["balance"] >= item["price"]:
                ...
                await bot.send_message(
                    ADMIN_ID,
                    f"🔔 @{callback.from_user.username} купил товар: {item['name']}"
                )
            else:
